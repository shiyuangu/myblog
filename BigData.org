...
#On the driver: Noting happens. Why? RDD.foreach is an action. It prints to stdout of the executors but println return Unit.
#+END_SRC
...
reduceByKey could be more efficient than groupByKey since it reduces the amount of data requiring shuffle. 
***** mapValues 
...
It's common to have filter and join. 
...
3. Untyped: The elements within DataFrames are Rows, which are not parameterized by a type. Therefore, the scala complier cannot type check Spark SQL schemas in DataFrames. 
